...
import { BulkAssignmentBar, BulkAssignmentHeaderCheckbox, BulkAssignmentRowCheckbox, BulkAssignmentSelectionProvider } from "@/components/bulk-assignment-controls";
...
import { buildQueueCaseHref, buildQueueHref, nextQueueSortDirection, type QueuePendenciaFilter, type QueueSlaBucket, type QueueSortDirection, type QueueSortKey } from "@/lib/queue";
...
export function BulkAssignmentQueue({
  ...
  const eligibleIds = sentences.filter((sentence) => isBulkAssignable(sentence, stage)).map((sentence) => sentence.id);
  ...
  return (
    <BulkAssignmentSelectionProvider
      ...
      <div className="space-y-3">
        ...
        <div className="overflow-hidden border border-zinc-800 bg-[#1d1e1c]">
          <div className="overflow-x-auto">
            <table className={`${showOrderSummary ? "min-w-[1590px]" : "min-w-[1250px]"} w-full table-fixed text-left text-xs`}>
                ...
                {sentences.map((sentence) => {
                  ...
                  return (
                    <ClickableTableRow
                      ...
                      className={`group cursor-pointer transition-colors hover:bg-sky-500/10 active:bg-sky-500/15 focus-visible:bg-sky-500/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-sky-500 [&:has(input[data-bulk-row-checkbox]:checked)]:bg-sky-500/10 ${
                    ...
                    >
                      <td className="px-4 py-3">
                        <div data-row-interactive="true">
                          <BulkAssignmentRowCheckbox eligible={eligible} sentenceId={sentence.id} processo={sentence.processo} />
                        </div>
                      </td>
