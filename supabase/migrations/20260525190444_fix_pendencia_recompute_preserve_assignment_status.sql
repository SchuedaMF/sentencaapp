  ...
  resolved as (
    select
      ...
      case
        ...
        when event_state.cumprimento_event_status is not null then event_state.cumprimento_event_status
        when s.cumprimento_base_status in ('ESTOQUE'::public.sentence_status, 'EM ANDAMENTO'::public.sentence_status) then
          case
            when nullif(btrim(coalesce(s.responsavel_cumprimento, '')), '') is null then 'ESTOQUE'::public.sentence_status
            else 'EM ANDAMENTO'::public.sentence_status
          end
        else s.cumprimento_base_status
      ...
      case
        when event_state.qualidade_event_status is not null then event_state.qualidade_event_status
        when s.qualidade_base_status in ('ESTOQUE'::public.sentence_status, 'EM ANDAMENTO'::public.sentence_status) then
          case
            when nullif(btrim(coalesce(s.responsavel_qualidade, '')), '') is null then 'ESTOQUE'::public.sentence_status
            else 'EM ANDAMENTO'::public.sentence_status
          end
        else s.qualidade_base_status
      end as qualidade_status,
...
update public.sentences s
   set qualidade_status = 'EM ANDAMENTO'::public.sentence_status
 where s.cumprimento_status = 'ENTREGUE'::public.sentence_status
   and s.qualidade_status = 'ESTOQUE'::public.sentence_status
   and s.qualidade_base_status in ('ESTOQUE'::public.sentence_status, 'EM ANDAMENTO'::public.sentence_status)
   and nullif(btrim(coalesce(s.responsavel_qualidade, '')), '') is not null
   and not exists (
     select 1
       from public.sentence_events e
      where e.sentence_id = s.id
        and e.etapa = 'QUALIDADE'
        and e.affects_operational_state
   );
