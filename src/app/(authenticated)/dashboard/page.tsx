...
function ProductionOverview({ metrics }: { metrics: DashboardMetrics }) {
  ...
  const showFullOperation = metrics.currentUser.role === "admin" || metrics.currentUser.role === "gestor";
  ...
  return (
    <>
      <div className="grid gap-4 px-5 pt-5 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Cumprimento no mês" value={production.month.cumprimento} accent="green" />
        <MetricCard label="Qualidade no mês" value={production.month.qualidade} accent="amber" />
      </div>
...
function MetricCard({ label, value, accent = "default" }: { label: string; value: number; accent?: MetricAccent }) {
  return (
    <section className="relative overflow-hidden border border-zinc-800 bg-[#1d1e1c] p-4">
      ...
      <div className="mt-2 font-mono text-3xl font-semibold text-zinc-100">
